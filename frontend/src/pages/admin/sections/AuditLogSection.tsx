...
export function AuditLogSection({ initialLogs, initialTotal }: AuditLogSectionProps) {
  ...
  const filteredLogs = useMemo(() => {
    if (!dateFrom && !dateTo) return displayLogs;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          size="sm"
          onClick={() => {
            const headers = ["Timestamp", "Action", "Entity", "EntityID", "Performer", "Details", "IP"];
            const rows = filteredLogs.map(e => [
              new Date(e.timestamp).toISOString(),
              e.action, e.entityType, e.entityId,
              e.performedBy, (e.details || "").replace(/,/g, ";"),
              e.ipAddress || "",
            ]);
      ...
      <ScrollArea className="h-[calc(100vh-22rem)]">
        <div className="space-y-2">
          ...
          {filteredLogs.map((entry) => {
            ...
            const truncatedId = entry.entityId.length > 12 ? `${entry.entityId.slice(0, 10)}…` : entry.entityId;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-muted-foreground font-mono text-xs" title={entry.entityId}>#{truncatedId}</span>
                      </div>
                      {entry.details && (
