        ...
        {devices.map((device) => {
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <FeatureIcon
                  name={icon.name}
                  size="xs"
                  className={`border-white/10 ${icon.accent}`}
                />
            ...
            <div className="space-y-1 text-sm text-white/75">
              <p>Battery <span className="font-semibold text-white">{device.battery}%</span></p>
              {device.heartRate && (
                <p>Heart rate <span className="font-semibold text-white">{device.heartRate} bpm</span></p>
              )}
              {device.temperature && (
                <p>Temperature <span className="font-semibold text-white">{device.temperature}°C</span></p>
              )}
            </div>
