      ...
      <div className="space-y-5">
        {itinerary.map((item, index) => (
          <div key={item.id} className="relative pl-6">
            {index !== 0 && <span className="absolute left-1.5 top-0 h-full w-px bg-white/15" aria-hidden />}
            <span
            ...
            <div className="flex flex-col gap-2 rounded-2xl border-l-4 border-white/15 bg-transparent px-4 py-2">
              ...
              <span
                className={`inline-flex w-max rounded-full px-3 py-0.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-white ${statusColors[item.status] || 'bg-slate-400'}`}
              >
                {item.status}
              </span>
            </div>
